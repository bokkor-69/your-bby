...
module.exports = {
  ...
  onStart: async function ({ event, api, commandName }) {
    ...
    return api.sendMessage("Please choose the visibility for the post:\n1. Everyone\n2. Friend\n3. Only me", threadID, (e, info) => {
  ...
  onReply: async function ({ Reply, event, api, commandName }) {
    ...
    if (type == "whoSee") {
      if (!["1", "2", "3"].includes(body)) return api.sendMessage('Please choose one of the options above', threadID, messageID);
      formData.input.audience.privacy.base_state = body == 1 ? "EVERYONE" : body == 2 ? "FRIENDS" : "SELF";
      api.unsendMessage(handleReply.messageID, () => {
        api.sendMessage('Please write the content for your post. If you don’t want to write anything, reply with 0.', threadID, (e, info) => {
          global.GoatBot.onReply.set(info.messageID, {
            commandName,
            messageID: info.messageID,
            author: author,
      ...
      api.unsendMessage(handleReply.messageID, () => {
        api.sendMessage('Please send an image or video (multiple attachments can be sent). If you don’t want to include any media, reply with 0.', threadID, (e, info) => {
          global.GoatBot.onReply.set(info.messageID, {
            commandName,
            messageID: info.messageID,
            author: author,
      ...
      api.httpPost('https://www.facebook.com/api/graphql/', form, (e, info) => {
        ...
        try {
          ...
          return api.sendMessage(`There was an issue creating the post, please try again later.`, threadID, messageID);
        }
      });
    }
  }
